...
ds_c = ds.traj.get_concentration(grid)
print(ds_c)
...
# Since pixels have the same size, this is proportional to the previous plot.
...
ds_c = ds.traj.get_concentration(grid3d, weights=weights)

#%% 
# Plot the oil concentration (mass/volume, kg/m3) at depths 1-3m and 20-30m
plt.subplot(1,2,1)
ds_c.mass_oil_volume_concentration.sel(z=-2).plot(vmin=0, vmax=4e-5)
plt.subplot(1,2,2)
ds_c.mass_oil_volume_concentration.sel(z=-25).plot(vmin=0, vmax=4e-5)
plt.show()
